...
import { CV, ExperienciaLaboral } from "../models/CV";
...
export function allFStimestampToDateObj(curriculum: CV) {
  curriculum.datosPersonales.fechaNacimiento = getJsDate(
    curriculum.datosPersonales.fechaNacimiento
  );
  curriculum.formacionBasica.fechaGraduacion = getJsDate(
    curriculum.formacionBasica.fechaGraduacion
  );

  if (curriculum.formacionSuperior) {
    for (let i = 0; i < curriculum.formacionSuperior.length; i++) {
      curriculum.formacionSuperior[i].fechaTerminacion = getJsDate(
        curriculum.formacionSuperior[i].fechaTerminacion
      );
    }
  }
  ...
  if (curriculum.experienciaLaboral) {
    for (let i = 0; i < curriculum.experienciaLaboral.length; i++) {
      curriculum.experienciaLaboral[i].fechaIngreso = getJsDate(
        curriculum.experienciaLaboral[i].fechaIngreso
      );
      if (curriculum.experienciaLaboral[i].fechaRetiro) {
        curriculum.experienciaLaboral[i].fechaRetiro = getJsDate(
          curriculum.experienciaLaboral[i].fechaRetiro
        );
      }
    }
  }
...
export function fromM2ToDate(curriculum: CV) {
  if (curriculum.datosPersonales.fechaNacimiento?.$d) {
    curriculum.datosPersonales.fechaNacimiento =
      curriculum.datosPersonales.fechaNacimiento?.$d;
  }
  if (curriculum.formacionBasica.fechaGraduacion?.$d) {
    curriculum.formacionBasica.fechaGraduacion =
      curriculum.formacionBasica.fechaGraduacion?.$d;
  }
  if (curriculum.formacionSuperior) {
    for (let i = 0; i < curriculum.formacionSuperior.length; i++) {
      if (curriculum.formacionSuperior[i].fechaTerminacion?.$d) {
        curriculum.formacionSuperior[i].fechaTerminacion =
          curriculum.formacionSuperior[i].fechaTerminacion?.$d;
      }
    }
  }
  if (curriculum.experienciaLaboral) {
    for (let i = 0; i < curriculum.experienciaLaboral.length; i++) {
      if (curriculum.experienciaLaboral[i].fechaIngreso?.$d) {
        curriculum.experienciaLaboral[i].fechaIngreso =
          curriculum.experienciaLaboral[i].fechaIngreso?.$d;
      }
      if (curriculum.experienciaLaboral[i].fechaRetiro?.$d) {
        curriculum.experienciaLaboral[i].fechaRetiro =
          curriculum.experienciaLaboral[i].fechaRetiro?.$d;
      }
    }
  }
}
